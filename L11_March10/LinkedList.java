...
import L11_March10.LinkedList.Node;
...
public class LinkedList {
	...
	public LinkedList Merge2SortedLL(LinkedList two) {
		LinkedList rv = new LinkedList();
		Node first = this.head;
		Node second = two.head;
		while (first != null && second != null) {
			if (first.data > second.data) {
				rv.addLast(second.data);
				second = second.next;
			} else {
				rv.addLast(first.data);
				first = first.next;
			}
		}
		if (first == null) {
	...
	public void MergeSort() {
		...
		LinkedList merged = fh.Merge2SortedLL(sh);
